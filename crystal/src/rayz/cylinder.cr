  ...
  class Cylinder < Shape
    ...
    def local_intersect_into(local_ray : Ray, buf : Array(Intersection)) : Nil
      ...
      if a.abs >= Util::EPSILON
        ...
        buf << Intersection.new(t0, self) if @minimum < y0 && y0 < @maximum

        y1 = local_ray.origin.y + t1 * local_ray.direction.y
        buf << Intersection.new(t1, self) if @minimum < y1 && y1 < @maximum
      end

      intersect_caps(local_ray, buf)
    end
    ...
    private def intersect_caps(ray : Ray, buf : Array(Intersection)) : Nil
      return unless @closed && ray.direction.y.abs >= Util::EPSILON

      t = (@minimum - ray.origin.y) / ray.direction.y
      buf << Intersection.new(t, self) if check_cap(ray, t)
