...
fn chapter7() {
    ...
    world.shape_mut(mid_id).set_transform(translation(-0.5, 1.0, 0.5));
    ...
    world.shape_mut(right_id).set_transform(translation(1.5, 0.5, -0.5) * scaling(0.5, 0.5, 0.5));
    ...
    world.shape_mut(left_id).set_transform(translation(-1.5, 0.33, -0.75) * scaling(0.33, 0.33, 0.33));
...
fn chapter8() {
    ...
    // Floor with checkers
    let floor_id = world.add(ShapeNode::plane());
    world.shape_mut(floor_id).material.pattern =
        Some(Pattern::checkers(Color::new(1.0, 1.0, 1.0), Color::new(0.2, 0.2, 0.2)));
    world.shape_mut(floor_id).material.specular = 0.0;
...
fn chapter9() {
    ...
    world.shape_mut(left_wall_id).set_transform(
        translation(0.0, 0.0, 5.0) * rotation_y(-PI / 4.0) * rotation_x(PI / 2.0),
    );
    ...
    world.shape_mut(right_wall_id).set_transform(
        translation(0.0, 0.0, 5.0) * rotation_y(PI / 4.0) * rotation_x(PI / 2.0),
    );
...
fn chapter10() {
    ...
    let floor_id = world.add(ShapeNode::plane());
    world.shape_mut(floor_id).material.pattern =
        Some(Pattern::checkers(Color::new(0.15, 0.15, 0.15), Color::new(0.85, 0.85, 0.85)));
    ...
    let back_wall_id = world.add(ShapeNode::plane());
    world.shape_mut(back_wall_id).set_transform(rotation_x(PI / 2.0) * translation(0.0, 0.0, 5.0));
    ...
    let left_id = world.add(ShapeNode::sphere());
    world.shape_mut(left_id).set_transform(translation(-1.5, 0.5, -0.5) * scaling(0.5, 0.5, 0.5));
...
fn chapter11() {
    ...
    let mut world = World::new();
    world.light = Some(Light::point(Point::new(2.0, 10.0, -5.0), Color::new(0.9, 0.9, 0.9)));

    // Room - large cube
    let room_id = world.add(ShapeNode::cube());
    world.shape_mut(room_id).set_transform(scaling(15.0, 15.0, 15.0));
    world.shape_mut(room_id).material.pattern =
        Some(Pattern::checkers(Color::new(0.15, 0.15, 0.15), Color::new(0.25, 0.25, 0.25)));
    ...
    world.shape_mut(table_id).set_transform(translation(0.0, 3.1, 0.0) * scaling(3.0, 0.1, 2.0));
    ...
    for &(x, z) in &[(-2.7_f64, -1.7_f64), (2.7, -1.7), (-2.7, 1.7), (2.7, 1.7)] {
        ...
        world.shape_mut(leg_id).set_transform(translation(x, 1.5, z) * scaling(0.1, 1.5, 0.1));
    ...
    world.shape_mut(box2_id).set_transform(
        translation(1.0, 3.7, 0.3) * rotation_y(-PI / 6.0) * scaling(0.4, 0.7, 0.4),
    );
...
fn chapter12() {
    ...
    let floor_id = world.add(ShapeNode::plane());
    world.shape_mut(floor_id).material.pattern =
        Some(Pattern::checkers(Color::new(0.5, 0.5, 0.5), Color::new(0.75, 0.75, 0.75)));
    ...
    for &(x, z) in &[(-2.0_f64, 1.5_f64), (2.0, 1.5), (-2.0, -1.5), (2.0, -1.5)] {
        ...
        world.shape_mut(leg_id).set_transform(translation(x, 0.0, z) * scaling(0.2, 1.0, 0.2));
    ...
    world.shape_mut(table_id).set_transform(translation(0.0, 3.0, 0.0) * scaling(2.5, 1.0, 1.5));
    ...
    world.shape_mut(candle_id).set_transform(translation(0.0, 3.2, -0.5) * scaling(0.1, 1.0, 0.1));
    world.shape_mut(candle_id).material.color = Color::new(0.9, 0.9, 0.7);

    // Open cylinder vase
    let vase_id = world.add(ShapeNode::cylinder_truncated(0.0, 1.0));
    world.shape_mut(vase_id).set_transform(translation(-1.0, 3.2, 0.0) * scaling(0.3, 0.8, 0.3));
...
fn chapter13() {
    ...
    let floor_id = world.add(ShapeNode::plane());
    world.shape_mut(floor_id).material.pattern =
        Some(Pattern::checkers(Color::new(0.9, 0.9, 0.9), Color::new(0.1, 0.1, 0.1)));
    world.shape_mut(floor_id).material.reflective = 0.1;

    // Tree: group containing trunk + foliage
    let tree_id = world.add(ShapeNode::group());
    world.shape_mut(tree_id).set_transform(translation(-2.0, 0.0, 2.0));

    let trunk_id = world.add_child(tree_id, ShapeNode::cylinder_closed(0.0, 1.5));
    world.shape_mut(trunk_id).set_transform(scaling(0.2, 1.0, 0.2));
    ...
    world.shape_mut(foliage_id).set_transform(translation(0.0, 1.5, 0.0));

    for (i, &(dy, s)) in [(0.0_f64, 0.9_f64), (0.5, 0.7), (1.0, 0.5)].iter().enumerate() {
    ...
    world.shape_mut(snowman_id).set_transform(translation(2.0, 0.0, 1.0));

    let body_id = world.add_child(snowman_id, ShapeNode::sphere());
    world.shape_mut(body_id).set_transform(translation(0.0, 0.75, 0.0) * scaling(0.75, 0.75, 0.75));
    world.shape_mut(body_id).material.color = Color::new(0.9, 0.9, 0.9);

    let head_id = world.add_child(snowman_id, ShapeNode::sphere());
    world.shape_mut(head_id).set_transform(translation(0.0, 1.85, 0.0) * scaling(0.4, 0.4, 0.4));
...
fn chapter14() {
    ...
    world.shape_mut(cone1_id).set_transform(translation(-2.0, 1.0, 0.0) * scaling(0.5, 1.0, 0.5));
    ...
    world.shape_mut(cone2_id).set_transform(translation(0.0, 1.0, 0.0));
    ...
    world.shape_mut(cone3_id).set_transform(translation(2.0, 1.0, 0.0) * scaling(0.5, 1.0, 0.5));
...
fn chapter15() {
    ...
    for (pa, pb) in &[(p1, p2), (p2, p3), (p3, p4), (p4, p1)] {
        ...
        world.shape_mut(t_id).set_transform(translation(-3.0, 0.0, 0.0));
    ...
    for (a, b, c) in &[
        (top, fr, rt), (top, rt, bk), (top, bk, lt), (top, lt, fr),
        (bot, rt, fr), (bot, bk, rt), (bot, lt, bk), (bot, fr, lt),
    ] {
        let t_id = world.add(ShapeNode::triangle(*a, *b, *c));
        world.shape_mut(t_id).set_transform(translation(1.0, 1.0, 0.0));
...
fn chapter16() {
    ...
    let floor_id = world.add(ShapeNode::plane());
    world.shape_mut(floor_id).material.pattern =
        Some(Pattern::checkers(Color::new(0.8, 0.8, 0.8), Color::new(0.2, 0.2, 0.2)));
    ...
    world.shape_mut(inner_sphere).set_transform(scaling(1.2, 1.2, 1.2));
    ...
    world.shape_mut(csg1_id).set_transform(translation(-2.0, 1.0, 0.0));

    // CSG union: two overlapping spheres
    let sph1 = world.add_shape(ShapeNode::sphere());
    world.shape_mut(sph1).set_transform(translation(0.0, 0.3, 0.0));
    ...
    world.shape_mut(sph2).set_transform(translation(0.0, -0.3, 0.0));
    ...
    world.shape_mut(csg2_id).set_transform(translation(1.5, 1.0, 0.0));
    ...
    world.shape_mut(sph3).set_transform(scaling(1.35, 1.35, 1.35));
    world.shape_mut(sph3).material.color = Color::new(0.3, 0.7, 0.3);

    let csg3_id = world.add(ShapeNode::csg_intersection(cube3, sph3));
    world.shape_mut(csg3_id).set_transform(translation(-0.5, 1.0, 2.5) * rotation_y(PI / 4.0));
...
fn chapter17() {
    ...
    for (a, b, c, na, nb, nc, col) in &[
        (sv[0], sv[1], sv[3], n_apex, n_base, n_base, Color::new(1.0, 0.3, 0.3)),
        (sv[1], sv[2], sv[3], n_base, n_apex, n_base, Color::new(0.3, 1.0, 0.3)),
        (sv[2], sv[0], sv[3], n_base, n_base, n_apex, Color::new(0.3, 0.3, 1.0)),
    ] {
