"""Chapter 9: Planes — infinite flat surfaces with spheres."""
...
from rayz.transformations import rotation_x, rotation_y, scaling, translation, view_transform
...
def run() -> None:
    ...
    left_wall = Plane()
    left_wall.set_transform(translation(0, 0, 5) * rotation_y(-math.pi / 4) * rotation_x(math.pi / 2))
    left_wall.material.color = Color(1, 0.9, 0.9)
    left_wall.material.specular = 0
    world.objects.append(left_wall)

    right_wall = Plane()
    right_wall.set_transform(translation(0, 0, 5) * rotation_y(math.pi / 4) * rotation_x(math.pi / 2))
    ...
    print("Rendering 200x100 scene with infinite planes...")
    canvas = camera.render_parallel(world)

    out_path = os.path.join(os.path.dirname(__file__), "chapter9.ppm")
    print(f"Writing {out_path}...", end="", flush=True)
    with open(out_path, "w") as f:
        f.write(canvas.to_ppm())
    print(" done.")
    print("Infinite planes scene rendered.")
    print("\n" + "=" * 60 + "\n")
