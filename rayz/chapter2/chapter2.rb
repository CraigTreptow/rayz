class Chapter2
  def self.run
    start = Point.new(x: 0.0, y: 1.0, z: 0.0)
    velocity = Vector.new(x: 1.0, y: 1.8, z: 0.0).normalize * 11.25
    projectile = Projectile.new(position: start, velocity: velocity)

    # gravity -0.1 unit/tick, and wind is -0.01 unit/tick
    gravity = Vector.new(x: 0.0, y: -0.1, z: 0.0)
    wind = Vector.new(x: -0.01, y: 0.0, z: 0.0)
    environment = Environment.new(gravity: gravity, wind: wind)

    canvas = Canvas.new(width: 900, height: 550)

    red = Color.new(red: 1.0, green: 0.0, blue: 0.0)
